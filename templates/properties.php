...
<section class="page-section"><div class="container"><div class="property-filter"><button class="active" data-property-filter="all">Tất cả</button><button data-property-filter="for sale">Đang bán</button><button data-property-filter="sold">Đã bán</button><input type="search" data-property-search placeholder="Tìm theo thành phố hoặc địa chỉ" /></div><div class="listing-grid property-list"><?php echo ethan_dao_vanilla_render_property_list(); ?></div></div></section><section class="newsletter"><div class="newsletter-inner reveal"><h2 class="h-section-title"><?php echo esc_html(ethan_mod('props_newsletter_title', 'Nhận tin')); ?> <span><?php echo esc_html(ethan_mod('props_newsletter_accent', 'nhà mới')); ?></span></h2><p><?php echo esc_html(ethan_mod('props_newsletter_body', 'Tôi gửi email khi thấy listing đáng xem hoặc giá khu vực bạn quan tâm thay đổi. Không spam.')); ?></p><form data-static-form class="newsletter-form"><div class="nested-input"><input type="email" placeholder="Địa chỉ email của bạn..." required /><button class="btn-ink"><?php echo esc_html(ethan_mod('props_newsletter_btn', 'Đăng ký nhận tin')); ?></button></div><p class="form-success" hidden><?php echo esc_html(ethan_mod('props_newsletter_success', 'Đã nhận. Tôi sẽ liên lạc sớm.')); ?></p></form></div></section></main>
